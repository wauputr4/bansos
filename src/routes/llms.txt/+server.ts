...
function validityLabel(item: (typeof bansosList)[number]) {
	if (item.validity.type === 'fixed' && item.validity.date) {
		return `berlaku sampai ${item.validity.date}`;
	}
	if (item.validity.type === 'forever') {
		return 'berlaku tanpa batas khusus';
	}
	return item.validity.description || 'masa berlaku tidak menentu';
}
...
function bansosLink(item: (typeof bansosList)[number]) {
	...
	const note = compact(
		`${item.provider}. ${item.description} Status: ${statusLabel(item.status)}, ${validityLabel(item)}.${tags}`
	);
